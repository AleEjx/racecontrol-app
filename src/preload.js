...
setNotifPosition: (pos) => ipcRenderer.invoke("overlay:set-notif-position", pos),
sendNotification: (message, type) => ipcRenderer.invoke("overlay:send-notification", { message, type }),
testNotification: () => ipcRenderer.invoke("overlay:test-notification"),
onNotification: (callback) => ipcRenderer.on("notif:incoming", (_e, payload) => callback(payload)),
...
overlayResizeBy:   (dw, dh) => ipcRenderer.invoke("overlay:resize-by", dw, dh),
  ...
  playPrankAudio: () => ipcRenderer.invoke('play-prank-audio'),
stopPrankAudio: () => ipcRenderer.invoke('stop-prank-audio'),
...
selectFuelSource: (id) => ipcRenderer.send('fuel-source-picker-result', id),
  ...
  onUpdateAvailable:  (cb) => ipcRenderer.on("update-available",  (_e, v, isBugfix) => cb(v, isBugfix)),
onUpdateDownloaded: (cb) => ipcRenderer.on("update-downloaded", (_e, v, isBugfix) => cb(v, isBugfix)),
  showPrank: () => ipcRenderer.invoke('show-prank'),
  prankCaptureInput: () => ipcRenderer.invoke('prank-capture-input'),
  hidePrank: () => ipcRenderer.invoke('hide-prank'),
